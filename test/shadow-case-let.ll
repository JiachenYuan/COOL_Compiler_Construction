...
@global_str.0 = internal constant [19 x i8] c"shadow-case-let.cl\00"
...
ok.0:
	%vtpm.3 = getelementptr %String, %String* @String.1, i32 0, i32 0
	%vtpm.4 = load %_String_vtable*, %_String_vtable** %vtpm.3
	%vtpm.5 = getelementptr %_String_vtable, %_String_vtable* %vtpm.4, i32 0, i32 0
...
case.3.0:
	%vtpm.7 = icmp slt i32 %vtpm.6, 3
	br i1 %vtpm.7, label %br_exit.3.0, label %src_gte_br.3.0

src_gte_br.3.0:
	%vtpm.8 = icmp sgt i32 %vtpm.6, 3
	br i1 %vtpm.8, label %br_exit.3.0, label %src_lte_mc.3.0

src_lte_mc.3.0:
	store %String* @String.1, %String** %vtpm.0
	store %String* @String.2, %String** %vtpm.1
	%vtpm.9 = load %String*, %String** %vtpm.1
...
ok.1:
	...
	%tmp.2 = load %Main* (%Main*,%String*) *, %Main* (%Main*,%String*) ** %vtpm.14
	%vtpm.15 = call %Main*(%Main*, %String* ) %tmp.2( %Main* %vtpm.10, %String* %vtpm.9 )
	store %Main* %vtpm.15, %Main** %tmp.0
...
ok.2:
	...
	%tmp.4 = load %Main* (%Main*,%String*) *, %Main* (%Main*,%String*) ** %vtpm.20
	%vtpm.21 = call %Main*(%Main*, %String* ) %tmp.4( %Main* %vtpm.16, %String* @String.3 )
