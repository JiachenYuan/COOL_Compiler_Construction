; ModuleID = 'dispatch6_o.ll'
source_filename = "dispatch6_o.ll"
...
@global_str.0 = internal constant [15 x i8] c"dispatch6_o.cl\00"
...
entry:
  %vtpm.13.i = tail call i8* @malloc(i32 16)
...
Main_new.exit:                                    ; preds = %entry
  %vtpm.15.i = bitcast i8* %vtpm.13.i to %_Main_vtable**
  store %_Main_vtable* @_Main_vtable_prototype, %_Main_vtable** %vtpm.15.i, align 8
  %vtpm.16.i = getelementptr i8, i8* %vtpm.13.i, i64 8
  %0 = bitcast i8* %vtpm.16.i to %A**
  store %A* null, %A** %0, align 8
  tail call void @abort()
  unreachable
}
...
define i32 @Main_main(%Main* nocapture readonly %self) {
entry:
...
entry:
  %vtpm.13 = tail call i8* @malloc(i32 16)
  %malloc.null = icmp eq i8* %vtpm.13, null
  br i1 %malloc.null, label %abort, label %okay

okay:                                             ; preds = %entry
  %vtpm.14 = bitcast i8* %vtpm.13 to %Main*
  %vtpm.15 = bitcast i8* %vtpm.13 to %_Main_vtable**
  store %_Main_vtable* @_Main_vtable_prototype, %_Main_vtable** %vtpm.15, align 8
  %vtpm.16 = getelementptr i8, i8* %vtpm.13, i64 8
  %0 = bitcast i8* %vtpm.16 to %A**
  store %A* null, %A** %0, align 8
  ret %Main* %vtpm.14
