; ModuleID = 'dispatch4_o.ll'
source_filename = "dispatch4_o.ll"
...
entry:
  %vtpm.26.i = tail call i8* @malloc(i32 8)
...
Main_func.exit:                                   ; preds = %Main_main.exit
  ...
  %vtpm.48.i.i = bitcast i8* %vtpm.46.i.i to %_Alpha_vtable**
  store %_Alpha_vtable* @_Alpha_vtable_prototype, %_Alpha_vtable** %vtpm.48.i.i, align 8
  %vtpm.17.i = load %_Main_vtable*, %_Main_vtable** %vtpm.28.i, align 8
  ...
  %vtpm.19.i = tail call %Main* %tmp.1.i(%Main* nonnull %vtpm.27.i, i32 42)
  ret i32 0
}
...
Alpha_new.exit:                                   ; preds = %entry
  %vtpm.48.i = bitcast i8* %vtpm.46.i to %_Alpha_vtable**
  store %_Alpha_vtable* @_Alpha_vtable_prototype, %_Alpha_vtable** %vtpm.48.i, align 8
  ret i32 42
}
...
ok.0:                                             ; preds = %entry
  ...
  %tmp.0 = load i32 (%Main*)*, i32 (%Main*)** %vtpm.12, align 8
  %vtpm.13 = tail call i32 %tmp.0(%Main* nonnull %self)
  ...
  %vtpm.19 = tail call %Main* %tmp.1(%Main* nonnull %self, i32 %vtpm.13)
  %vtpm.21 = bitcast %Main* %self to %Object*
  ret %Object* %vtpm.21
...
entry:
  %vtpm.37 = tail call i8* @malloc(i32 8)
  %malloc.null = icmp eq i8* %vtpm.37, null
  br i1 %malloc.null, label %abort, label %okay

okay:                                             ; preds = %entry
  %vtpm.38 = bitcast i8* %vtpm.37 to %Beta*
  %vtpm.39 = bitcast i8* %vtpm.37 to %_Beta_vtable**
  store %_Beta_vtable* @_Beta_vtable_prototype, %_Beta_vtable** %vtpm.39, align 8
  ret %Beta* %vtpm.38
...
okay:                                             ; preds = %entry
  ...
  %vtpm.48 = bitcast i8* %vtpm.46 to %_Alpha_vtable**
  store %_Alpha_vtable* @_Alpha_vtable_prototype, %_Alpha_vtable** %vtpm.48, align 8
  ret %Alpha* %vtpm.47
