...
@String.3 = constant %String {
	%_String_vtable* @_String_vtable_prototype,
	i8* getelementptr ([9 x i8], [9 x i8]* @global_str.3, i32 0, i32 0)
}
...
entry:
	...
	%vtpm.2 = bitcast %String* @String.1 to i8*
	%vtpm.3 = bitcast %String* @String.1 to i8*
	%vtpm.4 = icmp eq i8* %vtpm.2, %vtpm.3
	br i1 %vtpm.4, label %true.0, label %false.0

true.0:
	%vtpm.5 = load %Main*, %Main** %vtpm.1
...
ok.0:
	...
	%tmp.0 = load %Main* (%Main*,%String*) *, %Main* (%Main*,%String*) ** %vtpm.9
	%vtpm.10 = call %Main*(%Main*, %String* ) %tmp.0( %Main* %vtpm.5, %String* @String.2 )
	store %Main* %vtpm.10, %Main** %vtpm.0
	br label %end.0

false.0:
	%vtpm.11 = load %Main*, %Main** %vtpm.1
...
ok.1:
	...
	%vtpm.16 = call %Main*(%Main*, %String* ) %tmp.1( %Main* %vtpm.11, %String* @String.3 )
	store %Main* %vtpm.16, %Main** %vtpm.0
	br label %end.0
...
entry:
	...
	%malloc.null = icmp eq %Main* %vtpm.23, null
	br i1 %malloc.null, label %abort, label %okay

okay:
	%vtpm.24 = getelementptr %Main, %Main* %vtpm.23, i32 0, i32 0
	store %_Main_vtable* @_Main_vtable_prototype, %_Main_vtable** %vtpm.24
