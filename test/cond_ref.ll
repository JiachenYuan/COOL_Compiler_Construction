...
@str.0 = internal constant [8 x i8] c"cond.cl\00"
...
false.0:
	...
	%tmp.7 = getelementptr %Main, %Main* %tmp.6, i32 0, i32 2
	%tmp.8 = load %B*, %B** %tmp.7
	%tmp.9 = bitcast %B* %tmp.8 to %A*
	store %A* %tmp.9, %A** %tmp.1
	br label %end.0

end.0:
	%tmp.10 = load %A*, %A** %tmp.1
...
okay:
	...
	store %A* null, %A** %tmp.24
	%tmp.25 = getelementptr %Main, %Main* %tmp.22, i32 0, i32 2
	store %B* null, %B** %tmp.25
	%tmp.26 = getelementptr %Main, %Main* %tmp.22, i32 0, i32 1
	store %A* null, %A** %tmp.26
	%tmp.27 = getelementptr %Main, %Main* %tmp.22, i32 0, i32 2
