...
Main_main.exit:                                   ; preds = %entry
  ...
  %0 = bitcast i8* %vtpm.25.i to %IO*
  %vtpm.18.i = tail call %IO* @IO_out_int(%IO* nonnull %0, i32 8)
  ret i32 0
}
...
entry:
  %vtpm.8 = icmp eq %Main* %self, null
  br i1 %vtpm.8, label %abort, label %ok.0

ok.0:                                             ; preds = %entry
  %vtpm.9 = getelementptr %Main, %Main* %self, i64 0, i32 0
  %vtpm.10 = load %_Main_vtable*, %_Main_vtable** %vtpm.9, align 8
  %vtpm.11 = getelementptr %_Main_vtable, %_Main_vtable* %vtpm.10, i64 0, i32 11
  %tmp.0 = load i32 (%Main*)*, i32 (%Main*)** %vtpm.11, align 8
  %vtpm.12 = tail call i32 %tmp.0(%Main* nonnull %self)
  %vtpm.16 = load %_Main_vtable*, %_Main_vtable** %vtpm.9, align 8
  %vtpm.17 = getelementptr %_Main_vtable, %_Main_vtable* %vtpm.16, i64 0, i32 8
  %tmp.1 = load %Main* (%Main*, i32)*, %Main* (%Main*, i32)** %vtpm.17, align 8
  %vtpm.18 = tail call %Main* %tmp.1(%Main* nonnull %self, i32 %vtpm.12)
  %vtpm.20 = bitcast %Main* %self to %Object*
  ret %Object* %vtpm.20
