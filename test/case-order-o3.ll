; ModuleID = 'case-order.ll'
source_filename = "case-order.ll"
...
ok.0.i:                                           ; preds = %entry
  ...
  %0 = bitcast i8* %vtpm.33.i to %IO*
  %vtpm.27.i = tail call %IO* @IO_out_string(%IO* nonnull %0, %String* nonnull @String.2)
  ret i32 0
}
...
ok.0:                                             ; preds = %entry
  ...
  %.not = icmp eq i32 %vtpm.10, 5
  br i1 %.not, label %case.exit.0, label %case.0.0

case.0.0:                                         ; preds = %ok.0
  %0 = icmp ugt i32 %vtpm.10, 5
  br i1 %0, label %abort, label %case.exit.0

case.exit.0:                                      ; preds = %case.0.0, %ok.0
  %String.1.sink = phi %String* [ @String.2, %ok.0 ], [ @String.1, %case.0.0 ]
  %vtpm.26 = getelementptr %_Object_vtable, %_Object_vtable* %vtpm.8, i64 1
  %1 = bitcast %_Object_vtable* %vtpm.26 to %Main* (%Main*, %String*)**
  %tmp.3 = load %Main* (%Main*, %String*)*, %Main* (%Main*, %String*)** %1, align 8
  %vtpm.27 = tail call %Main* %tmp.3(%Main* nonnull %self, %String* nonnull %String.1.sink)
