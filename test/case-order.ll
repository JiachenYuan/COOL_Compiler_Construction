...
@global_str.2 = internal constant [6 x i8] c"main\0A\00"
@String.2 = constant %String {
	%_String_vtable* @_String_vtable_prototype,
	i8* getelementptr ([6 x i8], [6 x i8]* @global_str.2, i32 0, i32 0)
}

@global_str.1 = internal constant [8 x i8] c"object\0A\00"
@String.1 = constant %String {
	%_String_vtable* @_String_vtable_prototype,
	i8* getelementptr ([8 x i8], [8 x i8]* @global_str.1, i32 0, i32 0)
}

@global_str.0 = internal constant [14 x i8] c"case-order.cl\00"
...
case.5.0:
	%vtpm.11 = icmp slt i32 %vtpm.10, 5
	br i1 %vtpm.11, label %br_exit.5.0, label %src_gte_br.5.0

src_gte_br.5.0:
	%vtpm.12 = icmp sgt i32 %vtpm.10, 5
	br i1 %vtpm.12, label %br_exit.5.0, label %src_lte_mc.5.0

src_lte_mc.5.0:
	%vtpm.13 = bitcast %Object* %vtpm.6 to %Main*
	store %Main* %vtpm.13, %Main** %vtpm.2
	%vtpm.14 = load %Main*, %Main** %vtpm.3
...
ok.1:
	...
	%tmp.2 = load %Main* (%Main*,%String*) *, %Main* (%Main*,%String*) ** %vtpm.18
	%vtpm.19 = call %Main*(%Main*, %String* ) %tmp.2( %Main* %vtpm.14, %String* @String.2 )
	store %Main* %vtpm.19, %Main** %tmp.0
...
case.0.0:
	%vtpm.20 = icmp slt i32 %vtpm.10, 0
	br i1 %vtpm.20, label %br_exit.0.0, label %src_gte_br.0.0

src_gte_br.0.0:
	%vtpm.21 = icmp sgt i32 %vtpm.10, 5
	br i1 %vtpm.21, label %br_exit.0.0, label %src_lte_mc.0.0

src_lte_mc.0.0:
	store %Object* %vtpm.6, %Object** %vtpm.1
...
ok.2:
	...
	%tmp.3 = load %Main* (%Main*,%String*) *, %Main* (%Main*,%String*) ** %vtpm.26
	%vtpm.27 = call %Main*(%Main*, %String* ) %tmp.3( %Main* %vtpm.22, %String* @String.1 )
	store %Main* %vtpm.27, %Main** %tmp.0
	br label %case.exit.0
...
case.exit.0:
	...
	%vtpm.28 = bitcast %Main* %tmp.4 to %Object*
	ret %Object* %vtpm.28
