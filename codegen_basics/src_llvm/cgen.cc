...
void CgenClassTable::code_main(){
  // TODO: add code here
  // std::cerr << "calling code_main" << std::endl;
  // Create global string
  Constant* _str = ConstantDataArray::getString(context, "Main.main() returned %d\n", true);
  GlobalVariable* str_global_var = new GlobalVariable(
    the_module,
    _str->getType(),
    true, 
  ...
  // std::cerr << "created global string" << std::endl;

  // Define a function main that has no parameters and returns an i32
  FunctionType *func_ret_type_i32 = FunctionType::get(Type::getInt32Ty(context), false);
  Function *llvm_main_func = Function::Create(func_ret_type_i32, Function::ExternalLinkage, "main", the_module);

  // std::cerr << "defined main function" << std::endl;
  // Define an entry basic block
  BasicBlock *entry_block = BasicBlock::Create(context, "entry", llvm_main_func);
  // std::cerr << "defined entry" << std::endl;
  // Call Main_main(). This returns int for phase 1, Object for phase 2
  builder.SetInsertPoint(entry_block);
  Function* main_main = the_module.getFunction("Main_main");
  // std::cerr << "got function Main_main" << std::endl;
  Value* main_result = builder.CreateCall(main_main, {});

  // std::cerr << "created call to Main_main" << std::endl;
  #ifdef MP3
  // MP3
  #else
  // MP2
  ...
  Value* str_ptr = builder.CreateGEP(_str->getType(), str_global_var, indices);

  // std::cerr << "calling GEP" << std::endl;
  // Call printf with the string address of "Main_main() returned %d\n"
  // and the return value of Main_main() as its arguments
  Function* printf_func = the_module.getFunction("printf");
  builder.CreateCall(printf_func, {str_ptr, main_result});
  // std::cerr << "calling printf" << std::endl;
...
Function *method_class::code(CgenEnvironment *env) {
  ...
  FunctionType* func_type = FunctionType::get(Type::getInt32Ty(env->context), false);
  Function* main_method= Function::Create(func_type, Function::ExternalLinkage, "Main_main", env->the_module);
  BasicBlock* entry_block = BasicBlock::Create(env->context, "entry", main_method);
  env->curr_function = main_method;
  env->builder.SetInsertPoint(entry_block);
  
  // Recursively generate code
  Value* ret_value = expr->code(env);

  // Return the function
  env->builder.CreateRet(ret_value);

  env->get_or_insert_abort_block(main_method);
...
Value *assign_class::code(CgenEnvironment *env) {
  ...
  // /TODO: add code here and replace `return nullptr`
  Value* value = expr->code(env);

  env->builder.CreateStore(value, env->find_in_scopes(name).second);
  return value;
}
...
Value *cond_class::code(CgenEnvironment *env) {
  ...
  // /TODO: add code here and replace `return nullptr`
  Function* curr_function = env->curr_function;
  BasicBlock* labelThen = BasicBlock::Create(env->context, env->new_label("true.", false), curr_function);
  BasicBlock* labelElse = BasicBlock::Create(env->context, env->new_label("false.", false), curr_function);
  BasicBlock* labelEnd = BasicBlock::Create(env->context, env->new_label("end.", false), curr_function);
  // std::cerr << "all blocks created" << std::endl;
  // Type* cond_type = then_exp->get_expr_ty(env);

  // std::cerr << "got cond_type" << std::endl;

  
  // std::cerr << "1" << std::endl;

  Value* pred_result = pred->code(env);
  env->builder.CreateCondBr(pred_result, labelThen, labelElse);
  //std::cerr << "2" << std::endl;

  env->builder.SetInsertPoint(labelThen);
  Value* then_result = then_exp->code(env);

  //std::cerr << "setting allocated" << std::endl;
  AllocaInst* allocated = env->insert_alloca_at_head(then_result->getType());

  env->builder.CreateStore(then_result, allocated);
  env->builder.CreateBr(labelEnd);

  //std::cerr << "3" << std::endl;

  env->builder.SetInsertPoint(labelElse);
  Value* else_result = else_exp->code(env);
  env->builder.CreateStore(else_result, allocated);
  env->builder.CreateBr(labelEnd);

  //std::cerr << "4" << std::endl;

  env->builder.SetInsertPoint(labelEnd);

  Value* ret = env->builder.CreateLoad(then_result->getType(), allocated);
...
Value *loop_class::code(CgenEnvironment *env) {
  ...
  Function* curr_function = env->curr_function;
  BasicBlock* labelWhile = BasicBlock::Create(env->context, env->new_label("while.", false), curr_function);
  BasicBlock* labelBody = BasicBlock::Create(env->context, env->new_label("loop_body.", false), curr_function);
  BasicBlock* labelLoopEnd = BasicBlock::Create(env->context, env->new_label("loop_end.", false), curr_function);
  env->builder.CreateBr(labelWhile);

  env->builder.SetInsertPoint(labelWhile);
  Value* pred_result = pred->code(env);
  env->builder.CreateCondBr(pred_result, labelBody, labelLoopEnd);

  env->builder.SetInsertPoint(labelBody);
  Value* body_result = body->code(env);
  env->builder.CreateBr(labelWhile);

  env->builder.SetInsertPoint(labelLoopEnd);
...
Value *let_class::code(CgenEnvironment *env) {
  ...
  // /TODO: add code here and replace `return nullptr`
  AllocaInst* allocated_var;
  if (type_decl->get_string() == "Int") {
    allocated_var = env->insert_alloca_at_head(Type::getInt32Ty(env->context));
  } else if (type_decl->get_string() == "Bool") {
    allocated_var = env->insert_alloca_at_head(Type::getInt1Ty(env->context));
  }

  env->open_scope();

  Value* var_init = init->code(env);

  if (var_init == nullptr) {
    if (type_decl->get_string() == "Int") {
      var_init = ConstantInt::get(Type::getInt32Ty(env->context), 0);
    } else {
      var_init = ConstantInt::get(Type::getInt1Ty(env->context), false);
    }
  }

  env->builder.CreateStore(var_init, allocated_var);
  // std::string TypeStr;
  // raw_string_ostream TypeStrStream(TypeStr);
  // allocated_var->getAllocatedType()->print(TypeStrStream);
  // std::cerr<< "aasfjiijwfaefej  " << TypeStr << std::endl;
  env->add_binding(identifier, allocated_var);
...
Value *plus_class::code(CgenEnvironment *env) {
  ...
  // /TODO: add code here and replace `return nullptr`
  Value* op1 = e1->code(env);
  Value* op2 = e2->code(env);
  return env->builder.CreateAdd(op1, op2);
}
...
Value *sub_class::code(CgenEnvironment *env) {
  ...
  Value* op1 = e1->code(env);
  Value* op2 = e2->code(env);
  return env->builder.CreateSub(op1, op2);
}
...
Value *mul_class::code(CgenEnvironment *env) {
  ...
  Value* op1 = e1->code(env);
  Value* op2 = e2->code(env);
  return env->builder.CreateMul(op1, op2);
}
...
Value *divide_class::code(CgenEnvironment *env) {
  ...
  BasicBlock* ok_label = BasicBlock::Create(env->context, env->new_ok_label(), env->curr_function);
  Value* is_divider_zero = env->builder.CreateICmpEQ(op2, ConstantInt::get(Type::getInt32Ty(env->context), 0));

  env->builder.CreateCondBr(is_divider_zero, env->get_or_insert_abort_block(env->curr_function), ok_label);

  env->builder.SetInsertPoint(ok_label);

  return env->builder.CreateSDiv(op1, op2);
...
Value *neg_class::code(CgenEnvironment *env) {
  ...
  return env->builder.CreateSub(ConstantInt::get(Type::getInt32Ty(env->context), 0), op1);
}
...
Value *lt_class::code(CgenEnvironment *env) {
  ...
  Value* op1 = e1->code(env);
  Value* op2 = e2->code(env);
  return env->builder.CreateICmpSLT(op1, op2);
}
...
Value *eq_class::code(CgenEnvironment *env) {
  ...
  Value* op1 = e1->code(env);
  Value* op2 = e2->code(env);
  return env->builder.CreateICmpEQ(op1, op2);
}
...
Value *leq_class::code(CgenEnvironment *env) {
  ...
  Value* op1 = e1->code(env);
  Value* op2 = e2->code(env);
  return env->builder.CreateICmpSLE(op1, op2);
}
...
Value *comp_class::code(CgenEnvironment *env) {
  ...
  // /TODO: add code here and replace `return nullptr`
  Value* ret = env->builder.CreateXor(ConstantInt::getTrue(env->context), e1->code(env));
  return ret;
}
...
Value *int_const_class::code(CgenEnvironment *env) {
  ...
  // /TODO: add code here and replace `return nullptr`
  int val = std::atoi(token->get_string().c_str());
  Value* ret = env->builder.getInt32(val);
  return ret;
}
...
Value *object_class::code(CgenEnvironment *env) {
  ...
  std::pair<llvm::Type *, llvm::Value *>  p = env->find_in_scopes(name);
  return env->builder.CreateLoad(p.first, p.second);
}
