...
public:
  ...
  CgenEnvironment(CgenNode *cur_class)
      : var_table(), cur_class(cur_class),
        ...
        builder(class_table.builder), the_module(class_table.the_module), block_count(0), tmp_count(0),
        ok_count(0) {
  ...
  std::pair<llvm::Type *, llvm::Value *> find_in_scopes(Symbol name) {
    llvm::Value* temp = var_table.find_in_scopes(name);
    return std::make_pair(((llvm::AllocaInst*) temp)->getAllocatedType(), temp);
  }
