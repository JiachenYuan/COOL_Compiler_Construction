...
@global_str.0 = internal constant [9 x i8] c"bools.cl\00"
...
entry:
  %vtpm.5.i = tail call i8* @malloc(i32 8)
  %malloc.null.i = icmp eq i8* %vtpm.5.i, null
...
Main_new.exit:                                    ; preds = %entry
  %vtpm.7.i = bitcast i8* %vtpm.5.i to %_Main_vtable**
  store %_Main_vtable* @_Main_vtable_prototype, %_Main_vtable** %vtpm.7.i, align 8
  ret i32 0
}
...
define i1 @Main_main(%Main* nocapture readnone %self) #0 {
entry:
  ret i1 true
}
...
entry:
  %vtpm.5 = tail call i8* @malloc(i32 8)
  %malloc.null = icmp eq i8* %vtpm.5, null
  br i1 %malloc.null, label %abort, label %okay

okay:                                             ; preds = %entry
  %vtpm.6 = bitcast i8* %vtpm.5 to %Main*
  %vtpm.7 = bitcast i8* %vtpm.5 to %_Main_vtable**
  store %_Main_vtable* @_Main_vtable_prototype, %_Main_vtable** %vtpm.7, align 8
  ret %Main* %vtpm.6
