...
@global_str.0 = internal constant [10 x i8] c"cond_o.cl\00"
...
ok.0:
	...
	%vtpm.11 = getelementptr %_Main_vtable, %_Main_vtable* %vtpm.10, i32 0, i32 11
	%tmp.0 = load i32 (%Main*) *, i32 (%Main*) ** %vtpm.11
	%vtpm.12 = call i32(%Main* ) %tmp.0( %Main* %vtpm.7 )
	%vtpm.13 = load %Main*, %Main** %vtpm.6
	%vtpm.14 = icmp eq %Main* %vtpm.13, null
	br i1 %vtpm.14, label %abort, label %ok.1
...
ok.1:
	...
	%tmp.1 = load %Main* (%Main*,i32) *, %Main* (%Main*,i32) ** %vtpm.17
	%vtpm.18 = call %Main*(%Main*, i32 ) %tmp.1( %Main* %vtpm.13, i32 %vtpm.12 )
	%vtpm.19 = load %Main*, %Main** %vtpm.6
