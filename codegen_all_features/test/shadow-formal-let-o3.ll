; ModuleID = 'shadow-formal-let.ll'
source_filename = "shadow-formal-let.ll"
...
@global_str.2 = internal constant [4 x i8] c"bad\00"
@String.2 = constant %String { %_String_vtable* @_String_vtable_prototype, i8* getelementptr inbounds ([4 x i8], [4 x i8]* @global_str.2, i32 0, i32 0) }
@global_str.1 = internal constant [5 x i8] c"good\00"
...
@global_str.0 = internal constant [21 x i8] c"shadow-formal-let.cl\00"
...
entry:
  %vtpm.30.i = tail call i8* @malloc(i32 8)
  %malloc.null.i = icmp eq i8* %vtpm.30.i, null
...
Main_method.exit:                                 ; preds = %entry
  %vtpm.31.i = bitcast i8* %vtpm.30.i to %Main*
  %vtpm.32.i = bitcast i8* %vtpm.30.i to %_Main_vtable**
  store %_Main_vtable* @_Main_vtable_prototype, %_Main_vtable** %vtpm.32.i, align 8
  %0 = bitcast i8* %vtpm.30.i to %IO*
  %vtpm.24.i = tail call %IO* @IO_out_string(%IO* nonnull %0, %String* nonnull @String.1)
  %vtpm.10.i = load %_Main_vtable*, %_Main_vtable** %vtpm.32.i, align 8
  ...
  %vtpm.12.i = tail call %Main* %tmp.1.i(%Main* nonnull %vtpm.31.i, %String* nonnull @String.3)
  ret i32 0
}
...
define %Object* @Main_method(%Main* %self, %String* nocapture readnone %var) {
entry:
  %vtpm.20 = icmp eq %Main* %self, null
  br i1 %vtpm.20, label %abort, label %ok.0

ok.0:                                             ; preds = %entry
  %vtpm.21 = getelementptr %Main, %Main* %self, i64 0, i32 0
  %vtpm.22 = load %_Main_vtable*, %_Main_vtable** %vtpm.21, align 8
  %vtpm.23 = getelementptr %_Main_vtable, %_Main_vtable* %vtpm.22, i64 0, i32 7
  %tmp.0 = load %Main* (%Main*, %String*)*, %Main* (%Main*, %String*)** %vtpm.23, align 8
  %vtpm.24 = tail call %Main* %tmp.0(%Main* nonnull %self, %String* nonnull @String.1)
...
entry:
  %vtpm.30 = tail call i8* @malloc(i32 8)
  %malloc.null = icmp eq i8* %vtpm.30, null
  br i1 %malloc.null, label %abort, label %okay

okay:                                             ; preds = %entry
  %vtpm.31 = bitcast i8* %vtpm.30 to %Main*
  %vtpm.32 = bitcast i8* %vtpm.30 to %_Main_vtable**
  store %_Main_vtable* @_Main_vtable_prototype, %_Main_vtable** %vtpm.32, align 8
  ret %Main* %vtpm.31
